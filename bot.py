...
# --- WEB DASHBOARD (AUTO-REFRESH REMOVED) ---
...
def send_welcome(message):
    ...
    bot.reply_to(
        ...
        "⚡ **Supported:**\n"
        "• YouTube / Shorts / Social Media links\n"
        "• Direct Video download links\n\n"
        "All uploaded videos are formatted for instant browser playback on Archive.org.",
        parse_mode="Markdown"
    )
...
def handle_universal_url(message):
    ...
    add_log(f"Received URL: {url}")
    status = bot.reply_to(message, "⚡ **Analyzing link & fetching playable video...**", parse_mode="Markdown")
    
    item_id = f"video_{uuid.uuid4().hex[:8]}"
    ...
    # Ensures standard web-playable H.264/AAC MP4 format
    ydl_opts = {
    ...
    try:
        ...
        try:
            ...
            # Fallback for binary links
            add_log(f"Falling back to direct stream for {url}")
        ...
        add_log(f"Download finished ({format_size(file_size)}). Uploading to Internet Archive...")
        ...
        # Upload with movies mediatype and web format metadata
...
if __name__ == "__main__":
    ...
    add_log("Bot online. Playable video engine ready.")
